...
interface BkashManualPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  bookingId: number;
  amount: number;
...
export default function BkashManualPaymentModal({
  isOpen,
  onClose,
  bookingId,
  amount,
  onSuccess,
}: BkashManualPaymentModalProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true);
      const response = await proxy.post("/payments", {
        bookingId,
        amount,
      ...
      if (response.data.success) {
        toast.success("bKash payment submitted. Awaiting admin approval.");
        setSenderNumber("");
        setTxId("");
        onSuccess();
        onClose();
      ...
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-100 p-6">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Send money then submit sender info and Tx ID.</p>
          </div>
          <button
            onClick={onClose}
